		...
		switch(stance.toLowerCase()) {
			case "attack_nearest":
			{
				AbstractEntity newTarget = entities.stream()
					.min(Comparator.comparingDouble(e -> e.getLocation().distanceSquared(ownerLoc)))
					.orElse(null);
				 if(newTarget == null) {
			...
			{
				AbstractEntity newTarget = entities.stream()
					.max(Comparator.comparingDouble(e -> e.getLocation().distanceSquared(ownerLoc)))
					.orElse(null);
				 if(newTarget == null) {
					 result.setValue(SkillResult.INVALID_TARGET);
					 break;
			...
			{
				AbstractEntity newTarget = entities.stream()
					.max(Comparator.comparingDouble(e -> e.getHealth()))
					.orElse(null);
				 if(newTarget == null) {
					 result.setValue(SkillResult.INVALID_TARGET);
					 break;
			...
			{
				AbstractEntity newTarget = entities.stream()
					.max(Comparator.comparingDouble(e -> e.getHealth() / e.getMaxHealth()))
					.orElse(null);
				 if(newTarget == null) {
					 result.setValue(SkillResult.INVALID_TARGET);
					 break;
