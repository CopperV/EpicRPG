		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(Location loc : ringPoinst)
					p.getWorld().spawnParticle(Particle.LARGE_SMOKE, loc, 0, 0, 0.5f, 0, speed);
				for(Location loc : trailPoints)
					p.getWorld().spawnParticle(Particle.SMOKE, loc, 0, 0, 0.5f, 0, speed);
	...
	private void spellEffect(Location point, List<Location> ringPoinst, List<Location> trailPoints) {
		...
		for(Location loc : ringPoinst) {
			for(double y = 0; y < 4; y += 0.5) {
				...
				p.getWorld().spawnParticle(Particle.LARGE_SMOKE, tmp, 1, 0.1f, 0.1f, 0.1f, 0.15);
			}
		}
		for(Location loc : trailPoints) {
			for(double y = 0; y < 4; y += 0.5) {
				Location tmp = loc.clone().add(0, y, 0);
				p.getWorld().spawnParticle(Particle.SMOKE, tmp, 1, 0.1f, 0.1f, 0.1f, 0.15);
			}
		}
		
		int radius = dr.getObszar();
		point.getWorld().getNearbyEntities(point, radius, 8, radius, e -> {
