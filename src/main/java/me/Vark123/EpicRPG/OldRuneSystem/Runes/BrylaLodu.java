...
public class BrylaLodu extends ARune{
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(int i = 0; i < 2; ++i) {
					...
					double z = vec.getZ()*t;
					loc.add(x,y,z);
					p.getWorld().spawnParticle(Particle.ITEM_SNOWBALL, loc,10,0,0,0,0);
					p.getWorld().spawnParticle(Particle.ITEM_SNOWBALL, loc,10,0.5F,0.5F,0.5F,0.05F);
					p.getWorld().spawnParticle(Particle.EXPLOSION, loc,10,0.1F,0.1F,0.1F,0.01F);
	...
	private void zadajDamage(Entity e) {
		((LivingEntity)e).addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 20*dr.getDurationTime(), 10));
		new BukkitRunnable() {
			int t = 1;
			...
			public void run() {
				...
				if(!e.isDead()) {
					p.getWorld().spawnParticle(Particle.ITEM_SNOWBALL, e.getLocation().add(0,1,0),30,0.5F,0.5F,0.5F,0);
