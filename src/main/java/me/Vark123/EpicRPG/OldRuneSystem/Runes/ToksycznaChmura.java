...
public class ToksycznaChmura extends ARune {
	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(int i = 0; i < 35; ++i) {
					...
					tmp.getWorld().spawnParticle(Particle.DUST, tmp.add(x,y,z), 1, 0, 0, 0, 0.1f, dust);
				}
		...
		new BukkitRunnable() {
			...
			PotionEffect effect = new PotionEffect(PotionEffectType.SLOWNESS, 20, 2);
			...
			public void run() {
				...
				loc.getWorld().getNearbyEntities(loc, dr.getObszar(), dr.getObszar(), dr.getObszar()).stream().filter(e -> {
					...
					e.getWorld().spawnParticle(Particle.DUST, e.getLocation().add(0,1,0), 12, 0.4f, 0.4f, 0.4f, 0.05f, dust);
