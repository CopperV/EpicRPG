...
public class Wtopienie extends ARune {

	public Wtopienie(ItemStackRune dr, Player p) {
		super(dr, p);
	}
	...
	private void playerEffect(RpgPlayer rpg) {
		...
		rpg.getModifiers().setWtopienie(true);
		...
		new BukkitRunnable() {
			...
			BossBar bar = Bukkit.createBossBar("§2§lWtopienie§f: "+(int)timer+" sekund", BarColor.BLUE, BarStyle.SEGMENTED_12);{
				bar.setVisible(true);
				bar.addPlayer(p);
			...
			public void run() {
				...
				bar.setTitle("§2§lWtopienie§f: "+(int)timer+" sekund");
				bar.setProgress(timer/time);
				--timer;
			}
		...
		new BukkitRunnable() {
			...
			public void run() {
				if(timer <= 0) {
					...
					rpg.getModifiers().setWtopienie(false);
					this.cancel();
					return;
				}
