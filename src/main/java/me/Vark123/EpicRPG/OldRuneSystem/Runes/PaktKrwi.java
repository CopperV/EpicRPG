...
public class PaktKrwi extends ARune {
	...
	public PaktKrwi(ItemStackRune dr, Player p) {
		super(dr, p);
		this.modifier1 = true;
	}
	...
	public void castSpell() {
		...
		modifiers.setPaktKrwi(true);
		modifiers.setPaktKrwiMeasure(true);
		modifiers.setPaktKrwiHp(0);
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if(timer <= 0) {
					...
					modifiers.setPaktKrwi(false);
					modifiers.setPaktKrwiMeasure(false);
					modifiers.setPaktKrwiHp(0);
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if(modifiers.hasPaktKrwiMeasure()) {
					...
					p.getWorld().spawnParticle(Particle.DUST, loc, 6, .6f, .6f, .6f, .1f, dust);
				} else {
					p.getWorld().spawnParticle(Particle.DUST, loc, 6, .6f, .6f, .6f, .1f, dust);
				}
			}
