		...
		new BukkitRunnable() {
			...
			public void run() {
				if(!casterInCastWorld() || !entityInCastWorld(e))
					return;
				p.getWorld().spawnParticle(Particle.DRIPPING_WATER, e.getLocation(),20,0.75F,1.9F,0.75F,0.5F);
				p.getWorld().spawnParticle(Particle.DRIPPING_WATER, e.getLocation().add(0, 1.5, 0),20,0.75F,1.9F,0.75F,0.5F);
