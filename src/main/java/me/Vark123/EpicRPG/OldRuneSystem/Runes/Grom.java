	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				Collection<Entity> tmpList = loc.getWorld().getNearbyEntities(loc, dr.getObszar(), 8, dr.getObszar());
				...
				tmpList.stream().filter(e -> {
					...
					RuneDamage.damageNormal(p, le, dr, (p,le,dr)->{
						PotionEffect pe = new PotionEffect(PotionEffectType.SLOWNESS, 20*25, 1);
...
//		List<Entity> tempList = new ArrayList<>();
		new BukkitRunnable() {
			...
			public void run() {
				double y = t;
				target.add(0, y, 0);
				p.getWorld().spawnParticle(Particle.ITEM_SNOWBALL, target, 5, 0.3F, 0.3F, 0.3F, 0.01F);
