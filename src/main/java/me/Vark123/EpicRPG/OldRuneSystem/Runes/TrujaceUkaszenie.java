	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				++t;
				loc.add(vec);
				p.getWorld().spawnParticle(Particle.DUST, loc, 14, 0.15f, 0.15f, 0.15f, 0.05f, dust);
					...
					RuneDamage.damageNormal(p, le, dr, (p, le, dr) ->{
						...
						new BukkitRunnable() {
							...
							public void run() {
								...
								p.getWorld().spawnParticle(Particle.DUST, le.getLocation(), 14, 0.3f, 0.4f, 0.3f, 0.05f, dust);
							}
						}.runTaskTimer(Main.getInstance(), 0, 20);
					});
					p.getWorld().spawnParticle(Particle.DUST, loc, 30, 0.6f, 0.6f, 0.6f, 0.1f, dust);
