	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				p.getWorld().spawnParticle(Particle.DUST, loc2, 3, 0.15f, 0.15f, 0.15f, 0.05f, dust);
				...
				loc.getWorld().getNearbyEntities(loc2, 3, 3, 3, e -> {
					...
					p.getWorld().spawnParticle(Particle.DUST, e.getLocation(), 10, 0.3f, 0.3f, 0.3f, 0.1f, dust);
				});
	...
	private void castSpellReverse(Vector vec, Location from) {
		new BukkitRunnable() {
			...
			public void run() {
				...
				p.getWorld().spawnParticle(Particle.DUST, loc, 3, 0.15f, 0.15f, 0.15f, 0.05f, dust);
