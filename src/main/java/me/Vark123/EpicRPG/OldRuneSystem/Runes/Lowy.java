...
public class Lowy extends ARune {

	public Lowy(ItemStackRune dr, Player p) {
		super(dr, p);
	}
	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				p.getWorld().spawnParticle(Particle.DUST, loc, 5,0.1F,0.1F,0.1F,0.05F, dust);
			...
			public void run() {
				...
				loc.getWorld().getNearbyEntities(loc, 2, 5, 2, e -> {
					...
					if(RuneDamage.damageNormal(p, le, dr, (p,le,dr)->{
						...
						le.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 20*dr.getDurationTime(), 7));
				...
				p.getWorld().spawnParticle(Particle.DUST, loc, 3,0.2F,0.2F,0.2F,0.1F, dust);
	...
	private void trapEffect(Location loc) {
		new BukkitRunnable() {
			...
			public void run() {
				...
				--timer;
				p.getWorld().spawnParticle(Particle.DUST, loc, 8,0.2F,0.2F,0.2F,0.1F, dust);
			}
