	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				p.getWorld().spawnParticle(Particle.FIREWORK, mid,10,0.5F,0.5F,0.5F,0.1F);
				...
				for(double theta = 0; theta <= (Math.PI*2); theta = theta + (Math.PI/(dr.getObszar()*2))) {
					...
					p.getWorld().spawnParticle(Particle.FIREWORK, tmp, 2, 0.05F, 0.05F, 0.05F, 0.05F);
				}
		...
		new BukkitRunnable() {
			...
			PotionEffect effect = new PotionEffect(PotionEffectType.SLOWNESS, 20, 1);
			@Override
			public void run() {
				if(timer <= 0 || !casterInCastWorld()) {
					this.cancel();
				...
				entities.stream().filter(e -> {
					...
					e.getWorld().spawnParticle(Particle.ANGRY_VILLAGER, e.getLocation().add(0, 1, 0),10,0.3F,0.3F,0.3F,0.05F);
				...
				if(entities.size()>tmp.size()) {
					p.getWorld().spawnParticle(Particle.ANGRY_VILLAGER, loc,20,0.5F,0.5F,0.5F,0.1F);
