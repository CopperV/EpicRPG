			...
			public void run() {
				...
				for(int i = 0; i < 2; ++i) {
					...
					for(Location point : stos) {
						p.getWorld().spawnParticle(Particle.SMOKE, point, 3, 0, 0, 0, 0.05F);
						p.getWorld().spawnParticle(Particle.DUST, point, 2, 0.15F, 0.15F, 0.15F, 0.1F,dust);
					}
					...
					loc.getWorld().getNearbyEntities(loc, 3, 3, 3, e -> {
						...
						p.getWorld().spawnParticle(Particle.SMOKE, e.getLocation().add(0,1,0), 13, 0.2f, 0.2f, 0.2f, 0.1f);
						p.getWorld().spawnParticle(Particle.DUST, e.getLocation().add(0,1,0), 15, 0.7F, 0.7F, 0.7F, 0.1F,dust);
