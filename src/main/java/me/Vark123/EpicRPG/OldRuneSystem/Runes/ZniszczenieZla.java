...
public class ZniszczenieZla extends ARune {

	public ZniszczenieZla(ItemStackRune dr, Player p) {
		super(dr, p);
	}

	@Override
	public void castSpell() {
		p.getWorld().playSound(p.getLocation(), Sound.ENTITY_ENDERMAN_STARE, 1, 1);
		new BukkitRunnable() {
			double t = 0;
			...
			public void run() {
				...
				double z = vec.getZ()*t;
				loc.add(x,y,z);
				p.getWorld().spawnParticle(Particle.FIREWORK, loc, 1, 0, 0, 0, 0);
				p.getWorld().spawnParticle(Particle.HAPPY_VILLAGER, loc, 10,0.4F,0.4F,0.4F,0.3F);
				...
				loc.getWorld().getNearbyEntities(loc, 4, 4, 4, e -> {
					...
					e.getWorld().playSound(loc, Sound.ENTITY_ENDERMAN_DEATH, 1, 1);
					this.cancel();
				});
