...
public class LodowaWlocznia extends ARune {

	private LinkedList<Location> stos;
	private List<Entity> shooted;
	
	public LodowaWlocznia(ItemStackRune dr, Player p) {
		super(dr, p);
		stos = new LinkedList<>();
	...
	public void castSpell() {
		p.getWorld().playSound(p.getLocation(), Sound.BLOCK_AMETHYST_CLUSTER_BREAK, 1.5f, 0.7f);
		new BukkitRunnable() {
			int t = 0;
			...
			public void run() {
				...
				for(int i = 0; i < 2; ++i) {
					...
					for(Location point : stos)
						p.getWorld().spawnParticle(Particle.WAX_OFF, point, 5, 0.05f, 0.05f, 0.05f, 0.04f);
					...
					loc.getWorld().getNearbyEntities(loc, 3, 3, 3, e -> {
						...
						p.getWorld().spawnParticle(Particle.WAX_OFF, e.getLocation().add(0,1,0), 15, 0.4f, 0.5f, 0.4f, 0.15f);
						p.getWorld().playSound(e.getLocation(), Sound.BLOCK_AMETHYST_BLOCK_HIT, 1, 0.7f);
					});
