		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				loc.getWorld().getNearbyEntities(loc, radius.getValue(), radius.getValue(), radius.getValue(), e -> {
					...
					p.getWorld().spawnParticle(Particle.WITCH, e.getLocation().clone().add(0, 1, 0), 8, 0.3, 0.8, 0.3, 0.07);
				...
				for(int i = 0; i <= Math.ceil(tmpRadius*tmpRadius)*3; ++i) {
					...
					p.getWorld().spawnParticle(Particle.DUST, tmpLoc, 1, .05, .05, .05, .02, dust);
				}
