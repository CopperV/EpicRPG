	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				++t;
				loc.add(vec);
				p.getWorld().spawnParticle(Particle.MYCELIUM, loc, 20, 0.2f, 0.2f, 0.2f, 0.15f);
				...
				loc.getWorld().getNearbyEntities(loc, 3, 4, 3, e -> {
					...
					RuneDamage.damageNormal(p, le, dr, (p, le, dr) ->{
						le.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 20*dr.getDurationTime(), 1));
						new BukkitRunnable() {
...
//								Bukkit.broadcastMessage("Test: "+end+" "+timer);
								...
								p.getWorld().spawnParticle(Particle.MYCELIUM, io.lumine.mythic.bukkit.BukkitAdapter.adapt(mob.getEyeLocation()), 10, 0.3f, 0.3f, 0.3f, 0.07f);
							}
						}.runTaskTimer(Main.getInstance(), 0, 20);
					});
					p.getWorld().spawnParticle(Particle.MYCELIUM, loc, 20, 0.4f, 0.4f, 0.4f, 0.3f);
