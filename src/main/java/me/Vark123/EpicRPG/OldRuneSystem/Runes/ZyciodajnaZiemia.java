...
public class ZyciodajnaZiemia extends ARune {
	...
	public ZyciodajnaZiemia(ItemStackRune dr, Player p) {
		super(dr, p);
	}
		...
		new BukkitRunnable() {
			...
			public void run() {
				debuff_tmp.stream().forEach(tmp -> {
					...
					tmp.getWorld().spawnParticle(Particle.DUST, tmp.getLocation(), 10, 0.5f, 0.5f, 0.5f, 0.1f,dust);
				});
			}
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				entities.stream().filter(e -> {
					...
					tmp.getWorld().spawnParticle(Particle.BLOCK, tmp.getLocation().add(0,1,0),2,0.3F,0.3F,0.3F,0.1F,data);
				});
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(double theta = 0; theta <= (Math.PI*2); theta = theta + (Math.PI/(dr.getObszar()*2))) {
					...
					p.getWorld().spawnParticle(Particle.BLOCK, tmp, 3, 0.2F, 0.2F, 0.2F, 0.15F, data);
				}
