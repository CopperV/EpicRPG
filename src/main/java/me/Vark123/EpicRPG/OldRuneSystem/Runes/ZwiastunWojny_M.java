...
public class ZwiastunWojny_M extends ARune {

	public ZwiastunWojny_M(ItemStackRune dr, Player p) {
		super(dr, p);
		this.modifier2 = true;
	}
	...
	private void castMeteor(Location src, Location dst) {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				loc.getWorld().spawnParticle(Particle.EXPLOSION_EMITTER, loc, 4, 0.15, 0.15, 0.15, 0);
				loc.add(vec);
			}
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(double theta = 0; theta <= Math.PI*2; theta += step) {
					...
					p.getWorld().spawnParticle(Particle.EXPLOSION_EMITTER, pos, 1, 0, 0, 0, 0);
				}
