...
//			double targetAngle = Math.toDegrees((double)targetDirection.angle(headDirection));
		...
		if(target != null) {
			...
			new BukkitRunnable() {
				...
				public void run() {
					...
					loc.add(dir);
					p.getWorld().spawnParticle(Particle.DUST, loc, 8, 0.3f, 0.3f, 0.3f, 0, dust);
	...
	private void spellEffect(Location loc, double dmg) {
		DustOptions dust = new DustOptions(Color.fromRGB(0, 85, 0), 1.25f);
		p.getWorld().spawnParticle(Particle.DUST, loc, dr.getObszar()*30, dr.getObszar()/2.0f, dr.getObszar()/4.0f, dr.getObszar()/2.0f, 0.2f, dust);
