		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(double theta = 0; theta <= (Math.PI*2); theta = theta + (Math.PI/(dr.getObszar()*2))) {
					...
					p.getWorld().spawnParticle(Particle.DUST, tmp, 2, 0.15F, 0.15F, 0.15F, 0.1F, dust);
				}
	...
	private void addEffect(Entity entity, Location loc, int time) {
		...
		switch(bounds) {
			...
			case 1:
				...
				break;
			case 2:
				PotionEffect effect = new PotionEffect(PotionEffectType.SLOWNESS, 20*time, 1);
				((LivingEntity)entity).addPotionEffect(effect);	
				break;
			case 3:
				new BukkitRunnable() {
					...
					public void run() {
						...
						p.getWorld().spawnParticle(Particle.DUST, entity.getLocation(), 20, 0.8F, 0.8F, 0.8F, 0.2F, dust);		
