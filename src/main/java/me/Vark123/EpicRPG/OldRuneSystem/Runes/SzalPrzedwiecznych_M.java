...
public class SzalPrzedwiecznych_M extends ARune {

	public SzalPrzedwiecznych_M(ItemStackRune dr, Player p) {
		super(dr, p);
		this.modifier1 = true;
	}
	...
	public void castSpell() {
		...
		p.getWorld().playSound(p.getLocation(), Sound.ENTITY_EVOKER_PREPARE_SUMMON, 1, 0.8f);
		modifiers.setSzalPrzedwiecznych_m(true);
		...
		new BukkitRunnable() {
			...
			public void run() {
				if(timer <= 0 || !casterInCastWorld()) {
					...
					modifiers.setSzalPrzedwiecznych_m(false);
