...
public class PorazenieElektryczne extends ARune {
		...
		new BukkitRunnable() {
			...
			public void run() {
				for(int i = 0; i < 2; ++i) {
					...
					double z = vec.getZ()*t;
					loc.add(x,y,z);
					p.getWorld().spawnParticle(Particle.FIREWORK, loc, 10,0.1F,0.1F,0.1F,0.01F);
					...
					loc.getWorld().getNearbyEntities(loc, 4, 4, 4, e -> {
						...
						if(RuneDamage.damageNormal(p, (LivingEntity)e, dr)) {
							entitiesList.add(e);
							spellEffect(e);
							p.getWorld().spawnParticle(Particle.FIREWORK, loc, 10,1F,1F,1F,0.2F);
			...
			public void run() {
				...
				lista.stream().filter(e -> {
					...
					if(RuneDamage.damageNormal(p, (LivingEntity)e, dr)) {
						...
						p.getWorld().spawnParticle(Particle.FIREWORK, loc2, 10,1F,1F,1F,0.2F);
						loc1 = loc2;
					}
				});
	...
	private void drawLine(Location loc1, Location loc2) {
		...
		for(double length = 0; length < distance; p1.add(vec), length += space) {
			loc1.getWorld().spawnParticle(Particle.FIREWORK, p1.getX(), p1.getY(), p1.getZ() , 10,0.1F,0.1F,0.1F,0.01F);
		}
	}
