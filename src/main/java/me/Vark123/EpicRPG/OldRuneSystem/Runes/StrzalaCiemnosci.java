...
public class StrzalaCiemnosci extends ARune {

	public StrzalaCiemnosci(ItemStackRune dr, Player p) {
		super(dr, p);
	}
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(int i = 0; i < 2; ++i) {
					...
					DustOptions dust = new DustOptions(Color.PURPLE, 2);
					p.getWorld().spawnParticle(Particle.DUST, loc, 5,0.5F,0.5F,0.5F,0.1F,dust);
					...
					loc.getWorld().getNearbyEntities(loc, 4, 4, 4, e -> {
						...
						p.getWorld().spawnParticle(Particle.DUST, loc, 5, 1F,1F,1F,0.4F,dust);
						p.getWorld().spawnParticle(Particle.LARGE_SMOKE, loc, 5, 0, 1F, 0, 0.05F);
