...
public class LodowaLanca extends ARune {

	public LodowaLanca(ItemStackRune dr, Player p) {
		super(dr, p);
	}
		...
		new BukkitRunnable() {
			...
			public void run() {
				for(int i = 0; i < 2; ++i) {
					...
					double z = vec.getZ()*t;
					loc.add(x,y,z);
					p.getWorld().spawnParticle(Particle.ITEM_SNOWBALL, loc,20,0.1F,0.1F,0.1F,0);
					p.getWorld().spawnParticle(Particle.ITEM_SNOWBALL, loc,20,0.5F,0.5F,0.5F,0.05F);
