...
public class WypedzanieZla extends ARune {

	public WypedzanieZla(ItemStackRune dr, Player p) {
		super(dr, p);
	}

	@Override
	public void castSpell() {
		p.getWorld().playSound(p.getLocation(), Sound.ENTITY_GHAST_DEATH, 1, 1);
		new BukkitRunnable() {
			double t = 0;
			...
			public void run() {
				...
				double z = vec.getZ()*t;
				loc.add(x,y,z);
				p.getWorld().spawnParticle(Particle.HAPPY_VILLAGER, loc, 1, 0, 0, 0, 0);
				p.getWorld().spawnParticle(Particle.HAPPY_VILLAGER, loc, 10,0.3F,0.3F,0.3F,0.25F);
				...
				loc.getWorld().getNearbyEntities(loc, 5, 5, 5, e -> {
					...
					loc.getWorld().playSound(loc, Sound.ENTITY_ENDERMAN_DEATH, 7, 1);
