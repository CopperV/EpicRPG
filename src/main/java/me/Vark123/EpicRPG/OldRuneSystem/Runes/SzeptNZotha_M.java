...
public class SzeptNZotha_M extends ARune {
	...
	public SzeptNZotha_M(ItemStackRune dr, Player p) {
		super(dr, p);
	}
	...
	public void spellEffect(Location loc) {
		...
		for(int i = 0; i < 40; ++i) {
			...
			Vector dir = new Vector(x,y,z).normalize().multiply(1.44);
	...
	public void castProjectile(Location src, Vector dir) {
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(int i = 0; i < 4; ++i) {
					p.getWorld().spawnParticle(Particle.DUST, tmpLoc, 3, .15, .15, .15, .03, projectileDust);
					tmpLoc.add(tmpVec);
				}
