...
public class ZewSmierci_H extends ARune {
	...
	public ZewSmierci_H(ItemStackRune dr, Player p) {
		super(dr, p);
	}
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				loc.getWorld().getNearbyEntities(loc, 3, 3, 3, e -> {
					...
					double amount = _p.getAttribute(Attribute.GENERIC_MAX_HEALTH).getValue()*0.05;
