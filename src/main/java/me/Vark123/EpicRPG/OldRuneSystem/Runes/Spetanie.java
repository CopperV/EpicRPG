...
public class Spetanie extends ARune {
	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			double t = 0;
			PotionEffect stun = new PotionEffect(PotionEffectType.SLOWNESS, 20*dr.getDurationTime(), 7);
			PotionEffect slow = new PotionEffect(PotionEffectType.SLOWNESS, 20*dr.getDurationTime(), 1);
			@Override
			public void run() {
				t+=0.65;
				for(double theta = 0; theta <= (Math.PI*2); theta = theta + (Math.PI/16)) {
					...
					p.getWorld().spawnParticle(Particle.LARGE_SMOKE, loc, 2,0.2F,0.2F,0.2F,0.05F);
					loc.add(0,0.25,0);
					p.getWorld().spawnParticle(Particle.LARGE_SMOKE, loc, 2,0.2F,0.2F,0.2F,0.05F);
					loc.add(0,0.25,0);
					p.getWorld().spawnParticle(Particle.LARGE_SMOKE, loc, 2,0.2F,0.2F,0.2F,0.05F);
