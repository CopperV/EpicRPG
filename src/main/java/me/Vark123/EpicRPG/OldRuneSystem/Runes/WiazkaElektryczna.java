...
public class WiazkaElektryczna extends ARune {
		...
		new BukkitRunnable() {
			...
			public void run() {
				for(int i = 0; i < 2; ++i) {
					...
					double z = vec.getZ()*t;
					loc.add(x,y,z);
					p.getWorld().spawnParticle(Particle.ENCHANT, loc,10,0.1F,0.1F,0.1F,0);
	...
	private void spellEffect(Entity e) {
		...
		for(double k = sLoc.getBlockY(); k>= loc.getBlockY(); k-=0.5) {
			...
			sLoc.add(x, y, z);
			p.getWorld().spawnParticle(Particle.FIREWORK, sLoc, 10,0.1F,0.1F,0.1F,0.05F);
			sLoc.subtract(x, y, z);
			temp+=0.5;
		}
			...
			public void run() {
				new BukkitRunnable() {
					...
					public void run() {
						...
						++t;
						p.getWorld().spawnParticle(Particle.FIREWORK, loc, 20,1F,1F,1F,0.2F);
