...
public class WiezyKrwi extends ARune {
		...
		if(target != null) {
			...
			new BukkitRunnable() {
				...
				public void run() {
					...
					for(int i = 0; i < 2; ++i) {
						loc.add(vec);
						p.getWorld().spawnParticle(Particle.DUST, loc, 5, 0.06f, 0.06f, 0.06f, 0.01f, dust);
	...
	private void drawLine(Location loc1, Location loc2) {
		...
		for(double length = 0; length < distance; p1.add(vec), length += space) {
			loc1.getWorld().spawnParticle(Particle.DUST, p1.getX(), p1.getY(), p1.getZ() , 10,0.1F,0.1F,0.1F,0.01F, dust);
		}
	}
