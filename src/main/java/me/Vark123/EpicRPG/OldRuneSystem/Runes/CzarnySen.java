...
public class CzarnySen extends ARune {
	...
	public CzarnySen(ItemStackRune dr, Player p) {
		super(dr, p);
		effect =  new PotionEffect(PotionEffectType.SLOWNESS, 20*dr.getDurationTime(), 10);
	}
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				pointInfos.forEach(info -> {
					...
					p.getWorld().spawnParticle(Particle.LARGE_SMOKE, location, 1, 0, 0, 0, 0);
					p.getWorld().spawnParticle(Particle.SMOKE, location, 0, vec.getX(), vec.getY(), vec.getZ(), 0.18f);
				});
		...
		if(RuneDamage.damageNormal(p, (LivingEntity) e, dr, (p, le, dr) -> {
			...
			new BukkitRunnable() {
				...
				public void run() {
					...
					loc.getWorld().spawnParticle(Particle.SMOKE, loc, 4, 0.2f, 0.2f, 0.2f, 0.03f);
					--timer;
				}
			}.runTaskTimer(Main.getInstance(), 0, 5);
		}));
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(double theta = 0; theta < 2*Math.PI; theta += space) {
					...
					Location target = loc.clone().add(x,0,z);
					target.getWorld().spawnParticle(Particle.LARGE_SMOKE, target, 3, 0.2f, 0.8f, 0.2f, 0.04f);
				}
