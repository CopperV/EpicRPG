	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				double z = vec.getZ()*t;
				loc.add(x,y,z);
				p.getWorld().spawnParticle(Particle.BLOCK, loc,40,0.4F,0.4F,0.4F,0.02F, Bukkit.createBlockData(Material.DIRT));
	...
	private void zadajDamage(Entity e) {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if(!e.isDead()) {
					p.getWorld().spawnParticle(Particle.BLOCK, e.getLocation().add(0,0.25,0),40,0.3F,0.2F,0.3F,0.05f,Bukkit.createBlockData(Material.DIRT));
				}
				t+=0.5;
			}
