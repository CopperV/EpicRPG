...
public class ElektrycznePrzeciazenie extends ARune {
	...
	public void castSpell() {
		...
		PotionEffect effect = new PotionEffect(PotionEffectType.SLOWNESS, 20*dr.getDurationTime(), 2);
			...
			public void run() {
				...
				loc.getWorld().getNearbyEntities(loc, 3, 3, 3, e -> {
					...
					RuneDamage.damageNormal(p, le, dr, (p, le, dr) ->{
						le.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 20*dr.getDurationTime(), 9));
						...
						new BukkitRunnable() {
							...
							public void run() {
								...
								p.getWorld().spawnParticle(Particle.FIREWORK, le.getLocation(), 15, 0.3f, 0.4f, 0.3f, 0.05f);
							}
