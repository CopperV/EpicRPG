...
public class Wtopienie_M extends ARune {

	public Wtopienie_M(ItemStackRune dr, Player p) {
		super(dr, p);
	}
	...
	private void playerEffect(RpgPlayer rpg) {
		...
		rpg.getModifiers().setWtopienie_m(true);
		...
		new BukkitRunnable() {
			...
			BossBar bar = Bukkit.createBossBar("§2§lWtopienie II§f: "+(int)timer+" sekund", BarColor.BLUE, BarStyle.SEGMENTED_12);{
				bar.setVisible(true);
				bar.addPlayer(p);
			...
			public void run() {
				...
				bar.setTitle("§2§lWtopienie II§f: "+(int)timer+" sekund");
				bar.setProgress(timer/time);
				--timer;
			}
		...
		new BukkitRunnable() {
			...
			public void run() {
				if(timer <= 0) {
					...
					rpg.getModifiers().setWtopienie_m(false);
					this.cancel();
					return;
				}
				...
				p.getWorld().spawnParticle(Particle.BLOCK, loc, 12, 0.5f, 1, 0.5f, 0.15f, data);
				--timer;
			}
