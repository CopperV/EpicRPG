		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(double theta = 0; theta <= (Math.PI*2); theta+=progress) {
					...
					p.getWorld().spawnParticle(Particle.BLOCK, loc, 5, 0.1F, 0.1F, 0.1F, 0.05F, Bukkit.createBlockData(Material.DIRT));
				...
				loc.getWorld().getNearbyEntities(loc, t, t, t, e -> {
					...
					((LivingEntity)e).addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 20*dr.getDurationTime(), 2));
