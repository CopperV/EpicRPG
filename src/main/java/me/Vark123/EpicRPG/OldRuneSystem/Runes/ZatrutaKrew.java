	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				++t;
				loc.add(vec);
				p.getWorld().spawnParticle(Particle.DUST, loc, 8, 0.2f, 0.2f, 0.2f, 0.15f, dust);
					...
					RuneDamage.damageNormal(p, le, dr, (p, le, dr) ->{
						new BukkitRunnable() {
							...
							public void run() {
								...
								p.getWorld().spawnParticle(Particle.DUST, le.getLocation().add(0, 1, 0), 7, 0.3f, 0.3f, 0.3f, 0.07f, dust);
							}
						}.runTaskTimer(Main.getInstance(), 0, 20);
					});
					p.getWorld().spawnParticle(Particle.DUST, loc, 15, 0.4f, 0.4f, 0.4f, 0.3f, dust);
