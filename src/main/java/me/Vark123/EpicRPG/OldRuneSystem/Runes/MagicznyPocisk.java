...
//			Vector vec = loc.getDirection().normalize();
			...
			public void run() {
				...
				VectorUtils.rotateAroundAxisX(v, (loc.getPitch() + 90) * Math.toRadians(1));
				VectorUtils.rotateAroundAxisZ(v, (loc.getPitch() - 45) * Math.toRadians(1));
	            VectorUtils.rotateAroundAxisY(v, -loc.getYaw() * Math.toRadians(1));
				loc.add(v);
				loc.getWorld().playSound(loc, Sound.BLOCK_LAVA_POP, 1, 1);
				DustOptions dust = new DustOptions(Color.PURPLE, 1);
				p.getWorld().spawnParticle(Particle.DUST, loc, 20, 0.2, 0.2, 0.2, 0.05, dust);
