		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				castLoc.subtract(new Vector(Math.cos(angle), 0, Math.sin(angle))
						.normalize()
						.multiply(0.5));
				p.getWorld().spawnParticle(Particle.LARGE_SMOKE, castLoc, 4, 0.2f, 0.2f, 0.2f, 0.02f);
	...
	private void spellEffect(Location loc, Entity target) {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				loc.add(vec);
				p.getWorld().spawnParticle(Particle.SMOKE, loc, 6, 0.15f, 0.15f, 0.15f, 0.01f);
				...
				loc.getWorld().getNearbyEntities(loc, 3, 3, 3, e -> {
					...
					p.getWorld().spawnParticle(Particle.LARGE_SMOKE, e.getLocation().add(0,1,0), 15, 0.3f, 0.5f, 0.3f, 0.1f);
				});
			}
		}.runTaskTimer(Main.getInstance(), 0, 1);
	}
