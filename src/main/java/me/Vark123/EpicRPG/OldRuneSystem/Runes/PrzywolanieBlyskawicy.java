	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				++timer;
				loc.getWorld().spawnParticle(Particle.FIREWORK, loc, 9, .2f, .3f, .2f, .05f);
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(double k = loc2.getY(); k >= loc.getY(); k-=0.5) {
					...
					loc2.add(x, y, z);
					p.getWorld().spawnParticle(Particle.FIREWORK, loc2, 10,0.1F,0.1F,0.1F,0.05F);
					loc2.subtract(x, y, z);
					temp+=0.5;
				}
				...
				for(double r = 0.5; r < dr.getObszar(); r += .5) {
					for(double theta = 0; theta < (Math.PI*2); theta += ((Math.PI*2)/(r*4))) {
						...
						loc.getWorld().spawnParticle(Particle.FIREWORK, loc.clone().add(x,0,z), 2, .05f, 0, .05f, .02f);
					}
				}
