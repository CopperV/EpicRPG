...
public class KrwawyBicz extends ARune {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(int i = 0; i < 6; ++i) {
					...
					p.getWorld().spawnParticle(Particle.DUST, tmp, 0, 0, 1, 0, speed, cloudDust);
				}
			}
	...
	private void createLine(Location loc1, Location loc2) {
		...
		for(double length = 0; length < distance; p1.add(vec), length += space) {
			loc1.getWorld().spawnParticle(Particle.DUST, p1.getX(), p1.getY(), p1.getZ() , 3,0.1F,0.1F,0.1F,0.01F, lineDust);
		}
	}
