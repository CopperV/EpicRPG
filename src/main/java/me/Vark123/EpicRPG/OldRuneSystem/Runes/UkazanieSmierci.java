...
public class UkazanieSmierci extends ARune {
		...
		new BukkitRunnable() {
			...
			public void run() {
				for(int i = 0; i < 2; ++i) {
					...
					p.getWorld().spawnParticle(Particle.LARGE_SMOKE, loc,10,0.6F,0.6F,0.6F,0.05F);
	...
	private void spellEffect(Entity e) {
		if(isHit()) {
			p.getWorld().spawnParticle(Particle.LARGE_SMOKE, e.getLocation(),20,1F,1F,1F,0.4F);
			...
			p.getWorld().spawnParticle(Particle.HAPPY_VILLAGER, e.getLocation(),20,1F,1F,1F,0.4F);
