		...
		new BukkitRunnable() {
			...
			public void run() {
				for(int i = 0; i < 2; ++i) {
					...
					double z = vec.getZ()*t;
					loc.add(x,y,z);
					p.getWorld().spawnParticle(Particle.ENCHANT, loc,1,0,0,0,0);
					p.getWorld().spawnParticle(Particle.ENCHANT, loc,10,0.5F,0.5F,0.5F,0.05F);
					...
					loc.getWorld().getNearbyEntities(loc, 4, 4, 4, e -> {
						...
						p.getWorld().spawnParticle(Particle.ENCHANT, e.getLocation().add(0, 1, 0), 10, 0.3F, 1F, 0.3F, 0.2F);
