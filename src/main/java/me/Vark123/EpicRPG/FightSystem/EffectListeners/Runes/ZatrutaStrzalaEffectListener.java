...
public class ZatrutaStrzalaEffectListener implements Listener {
	
	private static final DustOptions dust = new DustOptions(Color.fromRGB(77, 181, 96), 0.9f);
	...
	public void onMod(EpicAttackEvent e) {
		...
		if(!Utils.hasEntityBuff(damager, EpicModifierTypes.ZATRUTA_STRZALA))
			return;
	...
	public void onMod(EpicPostDamageEffectEvent e) {
		...
		DamageUtils.applyTimingDirectDamageEffect(
				damager, 
				victim,
				e.getFinalDamage() * 0.02,
				org.bukkit.damage.DamageType.ARROW,
				DamageCause.CUSTOM,
				loc -> {
					loc.getWorld().spawnParticle(Particle.DUST, loc.clone().add(0,1,0), 9,
							0.4, 0.4, 0.4, 0.04, dust);
