...
public class BlackrockRemoveEvent implements Listener {
	
	@EventHandler
	public void onCall(BlackrockEvent e) {
		if(e.isCancelled())
			return;
		if(!e.getOperation().equals(BlackRockOperations.REMOVE))
			return;
		BlackrockManager.getInstance().removePlayerDailyBlackrock(e.getPlayer());
	}
