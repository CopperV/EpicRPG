...
public class RunicznyEgzekutor_MSetListener implements Listener {
	...
	public void onAttack(EpicAttackEvent e) {
		...
		int amount = info.getSetCounts().getOrDefault("Runiczny_Egzekutor_M", 0);
		if(amount < 4)
			return;
	...
	public void onDamageEffectToHeal(EpicEffectEvent e) {
		...
		double hpToRestore = p.getAttribute(Attribute.GENERIC_MAX_HEALTH).getValue() * 0.2;
