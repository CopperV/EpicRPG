...
public class Poswiecenie extends ACastableRune {

	private Random rand = new Random();
	
	public Poswiecenie(RpgPlayer rpgPlayer, EpicRune rune) {
	...
	public void castSpell() {
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.POSWIECENIE,
				player,
				target -> {
					target.getWorld().playSound(target.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 1, 0.8f);
				}, 
				target -> {
				...
				new TimingRuneEffect(4, target -> {
					...
					_loc.getWorld().spawnParticle(Particle.ENTITY_EFFECT, _loc, 8, 
							0.4f, 0.8f, 0.4f, force, Color.fromRGB(0, 255, 255));

					force = rand.nextDouble(0, 0.07);
					_loc.getWorld().spawnParticle(Particle.SOUL_FIRE_FLAME, _loc, 6, 
							0.4f, 0.8f, 0.4f, force);
				}));
	}
