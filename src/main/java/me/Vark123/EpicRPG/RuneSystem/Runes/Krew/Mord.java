...
public class Mord extends ACastableRune {

	private Random rand = new Random();
	private PotionEffect potion;
	
	public Mord(RpgPlayer rpgPlayer, EpicRune rune) {
		super(rpgPlayer, rune);
		
		potion = new PotionEffect(PotionEffectType.SPEED, 20*rune.getDurationTime(), 2);
	}
	...
	public void castSpell() {
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.MORD,
				player,
				target -> {
					target.addPotionEffect(potion);
					target.getWorld().playSound(target.getLocation(), Sound.ENTITY_RAVAGER_ROAR, 1.2f, 1.33f);
				}, 
				target -> {
				...
				new TimingRuneEffect(4, target -> {
					...
					double force = rand.nextDouble(0.3, 0.6);
					_loc.getWorld().spawnParticle(Particle.ENTITY_EFFECT, _loc, 5,
							0.4, 0.8, 0.4, force, Color.fromRGB(192, 28, 28));
				}));
	}
