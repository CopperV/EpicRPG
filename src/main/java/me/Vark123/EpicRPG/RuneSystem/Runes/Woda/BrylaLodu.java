...
import me.Vark123.EpicRPG.RuneSystem.Effects.DamageFreezeEffect;
...
public class BrylaLodu extends ACastableRune {
	...
	public BrylaLodu(RpgPlayer rpgPlayer, EpicRune rune) {
		...
		particleBlockData = Material.FROSTED_ICE.createBlockData();
	}
	...
	public void castSpell() {
		...
		ProjectileRuneTemplate.castProjectile(
				...
				boundingBox,
				loc -> {
					loc.getWorld().playSound(loc, Sound.ENTITY_BAT_TAKEOFF, 1, 0.75f);
				}, 
				loc -> {
					loc.getWorld().spawnParticle(Particle.FALLING_DUST, loc, 8, 
							0.4f, 0.4f, 0.4f, 0.05f, particleBlockData);
				}, 
				hitCondition, 
				(loc, e) -> {
					e.getWorld().playSound(loc, Sound.ENTITY_PLAYER_HURT_FREEZE, 1, 0.8f);
