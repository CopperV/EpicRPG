...
public class OstatniBoj_M extends ACastableRune {

	private Random rand = new Random();
	
	public OstatniBoj_M(RpgPlayer rpgPlayer, EpicRune rune) {
	...
	public void castSpell() {
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.OSTATNI_BOJ_M,
				player,
				target -> {
					target.getWorld().playSound(target.getLocation(), Sound.EVENT_RAID_HORN, 100f, 0.7f);
				}, 
				target -> {
				...
				new TimingRuneEffect(4, target -> {
					...
					_loc.getWorld().spawnParticle(Particle.TRIAL_SPAWNER_DETECTION, _loc, 4,
							0.4, 0.8, 0.4, force);
					_loc.getWorld().spawnParticle(Particle.SMALL_FLAME, _loc, 6,
							0.4, 0.8, 0.4, force);
					_loc.getWorld().spawnParticle(Particle.FLAME, _loc, 3,
							0.4, 0.8, 0.4, force);
				}));
	}
