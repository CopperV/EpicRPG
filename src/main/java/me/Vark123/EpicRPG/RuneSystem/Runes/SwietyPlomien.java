...
public class SwietyPlomien extends ARune {

	public SwietyPlomien(ItemStackRune dr, Player p) {
		super(dr, p);
	}

	@Override
	public void castSpell() {
		p.getWorld().playSound(p.getLocation(), Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 1, 1.6f);
		new BukkitRunnable() {
			double t = 0;
			...
			public void run() {
				...
				p.getWorld().spawnParticle(Particle.SOUL_FIRE_FLAME, loc2, 8, 0.13f, 0.05f, 0.13f, 0.065f);
				...
				loc.getWorld().getNearbyEntities(loc2, 3, 3, 3, e -> {
					...
					p.getWorld().spawnParticle(Particle.SOUL_FIRE_FLAME, e.getLocation(), 20, 0.4f, 0.5f, 0.4f, 0.18f);
				});
