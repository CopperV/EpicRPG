...
public class ZewSmierci_M extends ACastableRune {
	...
	public ZewSmierci_M(RpgPlayer rpgPlayer, EpicRune rune) {
		...
		ProjectileRuneTemplate.castProjectile(
				...
				(loc, e) -> {
					if(enemyHitCondition.check(player, e)) {
						...
						double value = 0.02*rpgPlayer.getInfo().getLevel() + 0.03*stats.getFinalMana() + 0.07*stats.getFinalInteligencja();
