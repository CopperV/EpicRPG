...
public class ZyciodajnaZiemia_M extends ACastableRune {
	...
	public ZyciodajnaZiemia_M(RpgPlayer rpgPlayer, EpicRune rune) {
	...
	public void castSpell() {
		...
		double hpRegenAmount = 15 + 0.3 * level;
		int manaRegenAmount = 30 + (int) (0.55 * level);
		...
		TotemRuneTemplate.castTotem(
					...
					new BukkitRunnable() {
						...
						public void run() {
							...
							affected.stream()
								...
								.forEach(entity -> {
									Utils.unsetEntityBuff(entity, EpicModifierTypes.ZYCIODAJNA_ZIEMIA_M);
				...
				(loc, entity) -> {
					...
					if(Utils.hasEntityBuff(entity, EpicModifierTypes.ZYCIODAJNA_ZIEMIA_M))
						return;
					
					Utils.setEntityBuff(entity, EpicModifierTypes.ZYCIODAJNA_ZIEMIA_M);
					affected.add(entity);
