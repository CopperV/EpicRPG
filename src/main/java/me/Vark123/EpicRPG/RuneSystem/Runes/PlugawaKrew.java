			...
			public void run() {
				...
				for(Entity en : entities) {
					...
					if(!en.equals(p) && en instanceof LivingEntity) {
						...
						blooded.add(en);
						en.getLocation().getWorld().playSound(en.getLocation(), Sound.ENTITY_HOSTILE_BIG_FALL, 1, .4f);
						le = (LivingEntity) en;
							...
							Bukkit.getScheduler().runTaskLater(Main.getInstance(), ()->{
								new BukkitRunnable() {
									...
									public void run() {
										...
										if(t>9|| !casterInCastWorld() || !entityInCastWorld(en)) {
											this.cancel();
											return;
										}
										++t;
