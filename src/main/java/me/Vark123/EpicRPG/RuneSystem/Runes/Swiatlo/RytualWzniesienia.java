...
public class RytualWzniesienia extends ACastableRune {

	private Random rand = new Random();
	
	public RytualWzniesienia(RpgPlayer rpgPlayer, EpicRune rune) {
	...
	public void castSpell() {
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.RYTUAL_WZNIESIENIA,
				player,
				target -> {
				...
				new TimingRuneEffect(2, target -> {
					...
					for(int i = 0; i < 3; ++i) {
						...
						_loc.getWorld().spawnParticle(Particle.END_ROD, _loc.clone().add(x,0,z), 0,
								0, 1, 0, force);
					}
					for(int i = 0; i < 5; ++i) {
						double radius = rand.nextDouble(0.8);
						...
						_loc.getWorld().spawnParticle(Particle.TRIAL_SPAWNER_DETECTION_OMINOUS, _loc.clone().add(x,0,z), 0,
								0, 1, 0, force);
					}
				}));
	}
