...
public class Inkantacja extends ACastableRune {
	...
	public Inkantacja(RpgPlayer rpgPlayer, EpicRune rune) {
	...
	public void castSpell() {
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.INKANTACJA,
				player,
				target -> {
					target.getWorld().playSound(target.getLocation(), Sound.ENTITY_GENERIC_EXTINGUISH_FIRE, 1, 0.4f);
				}, 
				target -> {
				...
				new TimingRuneEffect(4, target -> {
					...
					_loc.getWorld().spawnParticle(Particle.FLAME, _loc, 2, 0.4f, 0.8f, 0.4f, force);
