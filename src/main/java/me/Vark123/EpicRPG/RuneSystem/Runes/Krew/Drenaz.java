...
public class Drenaz extends ACastableRune {
	...
	public Drenaz(RpgPlayer rpgPlayer, EpicRune rune) {
		...
		healValue = 0.02 * rpgPlayer.getInfo().getLevel() + 0.04 * stats.getFinalMana() + 0.1 * stats.getFinalInteligencja();
	}
	...
	public void castSpell() {
		...
		InstantRangeRuneTemplate.castEffect(
				...
				0,
				loc -> {
					loc.getWorld().playSound(loc, Sound.ENTITY_WITHER_AMBIENT, 1, 0.5f);
					
					loc.getWorld().spawnParticle(Particle.DUST, loc, 12, 0.4f, 0.6f, 0.4f, 0.33f, dust);
				}, 
				hitCondition, 
				(loc, entity) -> {
	...
	private void castProjectile(Location startLoc) {
		ProjectileRuneTemplate.castProjectile(
				...
				new BoundingBox(),
				loc -> {
					float pitch = random.nextFloat(0.3f, 0.7f);
					loc.getWorld().playSound(loc, Sound.ENTITY_WITHER_SHOOT, 1, pitch);
				},
				loc -> {
					loc.getWorld().spawnParticle(Particle.DUST, loc, 4, 0.2f, 0.2f, 0.2f, 0.2f, dust);
				}, 
				emptyCondition,
				(loc, entity) -> { },
				loc -> {
	...
	private void castMissile(Location startLoc) {
		MissileRuneTemplate.castMissile(
				...
				(loc, entity) -> {
					...
					RpgPlayerHealEvent event = new RpgPlayerHealEvent(rpg, healValue);
					Bukkit.getPluginManager().callEvent(event);
					if(!event.isCancelled()) {
						loc.getWorld().playSound(loc, Sound.ENTITY_PLAYER_HURT_SWEET_BERRY_BUSH, 1, random.nextFloat(0.5f, 0.8f));
						
						Location loc2 = entity.getLocation().clone().add(0, 1, 0);
						loc2.getWorld().spawnParticle(Particle.HEART, loc2, 12,
								0.4f, 0.8f, 0.4f, 0.1);
					}
				},
				loc -> { });
