...
public class TajemnyGrad_M extends ARune {
	
	private static Random rand = new Random();

	public TajemnyGrad_M(ItemStackRune dr, Player p) {
		super(dr, p);
	}
	...
	private void castProjectile(Vector vec, Location src) {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(int i = 0; i < 6; ++i) {
					p.getWorld().spawnParticle(Particle.SPELL_WITCH, tmpLoc, 8, .15, .15, .15, .02);
					tmpLoc.add(tmpVec);
				}
