...
public class SzostyZmysl_H extends ACastableRune {

	private Random rand = new Random();
	
	public SzostyZmysl_H(RpgPlayer rpgPlayer, EpicRune rune) {
	...
	public void castSpell() {
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.SZOSTY_ZMYSL_H,
				player,
				target -> {
					target.getWorld().playSound(target.getLocation(), Sound.ENTITY_ILLUSIONER_MIRROR_MOVE, 1f, 0.7f);
				}, 
				target -> {
				...
				new TimingRuneEffect(4, target -> {
					...
					_loc.getWorld().spawnParticle(Particle.SCRAPE, _loc, 4,
							0.4, 0.8, 0.4, force);
					_loc.getWorld().spawnParticle(Particle.HAPPY_VILLAGER, _loc, 6,
							0.4, 0.8, 0.4, force);
					_loc.getWorld().spawnParticle(Particle.TOTEM_OF_UNDYING, _loc, 3,
							0.4, 0.8, 0.4, force);
				}));
	}
