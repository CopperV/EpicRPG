...
public class ToksycznaPulapka extends ARune {
	...
	private static final PotionEffect effect = new PotionEffect(PotionEffectType.SLOW, 20*12, 1);
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				for(int i = 0; i < points * 2; ++i) {
					...
					if(i % 2 == 0)
						loc.getWorld().spawnParticle(Particle.REDSTONE, tmp, 1, 0.05f, 0.05f, 0.05f, 0.02f, dustData);
					else
						loc.getWorld().spawnParticle(Particle.BLOCK_CRACK, tmp, 1, 0.05f, 0.05f, 0.05f, 0.02f, chainData);
				}
	...
	private void trapEffect(LivingEntity victim) {
		...
		
		{
			...
			for(int i = 0; i < points; ++i) {
				...
				tmp.getWorld().spawnParticle(Particle.REDSTONE, tmp, 1, 0.05f, 0.05f, 0.05f, 0.1f, dustData);
			}
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				vLoc.getWorld().spawnParticle(Particle.BLOCK_CRACK, vLoc, 12, 0.5f, 0.9f, 0.5f, 0.06f, chainData);
			}
