...
public class BarbarzynskiSzal extends ACastableRune {

	private Random rand = new Random();
	
	public BarbarzynskiSzal(RpgPlayer rpgPlayer, EpicRune rune) {
	...
	public void castSpell() {
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.BARBARZYNSKI_SZAL,
				player,
				target -> {
					target.getWorld().playSound(target.getLocation(), Sound.ENTITY_ZOMBIFIED_PIGLIN_ANGRY, 1.6f, 0.6f);
				}, 
				target -> {
				...
				new TimingRuneEffect(4, target -> {
					...
					_loc.getWorld().spawnParticle(Particle.ENTITY_EFFECT, _loc.clone().add(0,1,0),
	                		5, 0.4, 0.9, 0.4, 0.4, Color.fromRGB(192, 28, 28));
					for(int i = 0; i < 3; ++i) {
