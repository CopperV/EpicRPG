...
public class ZewSmierci_H extends ACastableRune {
	...
	public ZewSmierci_H(RpgPlayer rpgPlayer, EpicRune rune) {
		...
		ProjectileRuneTemplate.castProjectile(
				...
				(loc, e) -> {
					if(enemyHitCondition.check(player, e)) {
						...
						double value = 0.02*rpgPlayer.getInfo().getLevel() + 0.025*stats.getFinalMana() + 0.06*stats.getFinalInteligencja();
