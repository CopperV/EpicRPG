...
public class KrwawaStrzala extends ACastableRune {

	private Random rand = new Random();
	
	public KrwawaStrzala(RpgPlayer rpgPlayer, EpicRune rune) {
	...
	public void castSpell() {
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.KRWAWA_STRZALA,
				player,
				target -> {
				...
				new TimingRuneEffect(4, target -> {
					...
					double force = rand.nextDouble(0.25, 0.55);
					_loc.getWorld().spawnParticle(Particle.ENTITY_EFFECT, _loc, 5, 0.4f, 0.9f, 0.4f, force, Color.fromRGB(224, 32, 32));
				}));
	}
