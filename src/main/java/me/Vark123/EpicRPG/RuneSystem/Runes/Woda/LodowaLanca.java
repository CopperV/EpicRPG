...
public class LodowaLanca extends ACastableRune {

	private IRuneHitCondition hitCondition;
	private BoundingBox boundingBox;
	private BlockData particleBlockData;
	
	public LodowaLanca(RpgPlayer rpgPlayer, EpicRune rune) {
		...
		boundingBox = new BoundingBox(
				-0.7, -0.7, -0.7, 
				0.7, 0.7, 0.7);
		particleBlockData = Material.ICE.createBlockData();
	}
		...
		ProjectileRuneTemplate.castProjectile(
				...
				loc -> {
					loc.getWorld().spawnParticle(Particle.BLOCK, loc, 6, 
							0.08f, 0.08f, 0.08f, 0.01f, particleBlockData);
				}, 
				hitCondition, 
				(loc, e) -> {
					e.getWorld().playSound(loc, Sound.ENTITY_PLAYER_HURT_FREEZE, 1, 1.4f);
