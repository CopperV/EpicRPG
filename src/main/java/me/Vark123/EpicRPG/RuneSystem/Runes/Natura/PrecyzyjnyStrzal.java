...
public class PrecyzyjnyStrzal extends ACastableRune {

	private Random rand = new Random();
	
	public PrecyzyjnyStrzal(RpgPlayer rpgPlayer, EpicRune rune) {
	...
	public void castSpell() {
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.PRECYZYJNY_STRZAL,
				player,
				target -> {
				...
				new TimingRuneEffect(4, target -> {
					...
					double force = rand.nextDouble(0.03, 0.08);
					_loc.getWorld().spawnParticle(Particle.CRIT, _loc, 3, 0.4f, 0.9f, 0.4f, force);
				}));
	}
