...
public class SzalPrzedwiecznych_H extends ACastableRune {

	private Random rand = new Random();
	
	public SzalPrzedwiecznych_H(RpgPlayer rpgPlayer, EpicRune rune) {
	...
	public void castSpell() {
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.SZAL_PRZEDWIECZNYCH_H,
				player,
				target -> {
					target.getWorld().playSound(target.getLocation(), Sound.ENTITY_EVOKER_PREPARE_SUMMON, 1, 0.8f);
				}, 
				target -> {
				...
				new TimingRuneEffect(4, target -> {
					...
					_loc.getWorld().spawnParticle(Particle.DRAGON_BREATH, _loc, 3, 0.4f, 0.9f, 0.4f, force);
				}));
	}
