...
public class PaktKrwi extends ACastableRune {
	...
	private static final double PACT_HP = 300.;
	
	public PaktKrwi(RpgPlayer rpgPlayer, EpicRune rune) {
		super(rpgPlayer, rune);
	}

	@Override
	public void castSpell() {
		rpgPlayer.getModifiers().setPaktKrwiHp(PACT_HP);
		...
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.PAKT_KRWI_MEASURE,
				player,
				target -> {
				...
				target -> {
					...
					if(Utils.hasEntityBuff(target, EpicModifierTypes.PAKT_KRWI_MEASURE))
				...
				new TimingRuneEffect(2, target -> {
					if(rpgPlayer.getModifiers().getPaktKrwiHp() <= 0) {
						Utils.unsetEntityBuff(target, EpicModifierTypes.PAKT_KRWI_MEASURE);
