...
public class PaktKrwi_H extends ACastableRune {
	...
	private static final double PACT_HP = 360.;
	
	public PaktKrwi_H(RpgPlayer rpgPlayer, EpicRune rune) {
		super(rpgPlayer, rune);
	}

	@Override
	public void castSpell() {
		rpgPlayer.getModifiers().setPaktKrwiHp_h(PACT_HP);
		...
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.PAKT_KRWI_MEASURE_H,
				player,
				target -> {
				...
				target -> {
					...
					if(Utils.hasEntityBuff(target, EpicModifierTypes.PAKT_KRWI_MEASURE_H))
				...
				new TimingRuneEffect(2, target -> {
					if(rpgPlayer.getModifiers().getPaktKrwiHp_h() <= 0) {
						Utils.unsetEntityBuff(target, EpicModifierTypes.PAKT_KRWI_MEASURE_H);
					...
					hpBar.setProgress(Utils.limitValue(0, 1, 1 - rpgPlayer.getModifiers().getPaktKrwiHp_h() / PACT_HP));
				}));
	}
