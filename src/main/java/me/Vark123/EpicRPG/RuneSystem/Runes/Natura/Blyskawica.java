...
public class Blyskawica extends ACastableRune {
	
	private static final Random rand = new Random();
	private IRuneHitCondition hitCondition;
	private BoundingBox boundingBox;

	public Blyskawica(RpgPlayer rpgPlayer, EpicRune rune) {
	...
	public void castSpell() {
		...
		spellEffect(hit);
		hit.getWorld().playSound(hit, Sound.ENTITY_LIGHTNING_BOLT_IMPACT, 1.4f, 0.7f);
	...
	private void spellEffect(Location loc) {
		...
		for(int i = 0; i < count; ++i) {
            ...
            Utils.drawLine(Particle.FIREWORK, loc, next, 0.1, 3, 0.1f, 0.1f, 0.1f, 0.04f);
            Utils.drawLine(Particle.ELECTRIC_SPARK, loc, next, 0.07, 2, 0.07f, 0.07f, 0.07f, 0.02f);
