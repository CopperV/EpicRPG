...
public class BarbarzynskiSzal extends ARune {

	public BarbarzynskiSzal(ItemStackRune dr, Player p) {
		super(dr, p);
	}

	@Override
	public void castSpell() {
		p.getWorld().playSound(p.getLocation(), Sound.ENTITY_ZOMBIFIED_PIGLIN_ANGRY, 2, .6f);
		RpgPlayer rpg = Main.getListaRPG().get(p.getUniqueId().toString());
		rpg.setBarbarzynskiSzal(true);
		...
		new BukkitRunnable() {
			...
			DustOptions dust = new DustOptions(Color.fromRGB(192, 28, 28), 1.25f);
			@Override
			public void run() {
				--timer;
				...
				if(timer<0 || !casterInCastWorld()) {
					...
					rpg.setBarbarzynskiSzal(false);
					this.cancel();
					return;
				}
				p.getWorld().spawnParticle(Particle.REDSTONE, loc, 7, 0.5F, 0.5F, 0.5F, 0.1f, dust);
			}
