...
public class TarczaCienia extends ACastableRune {

	private Random rand = new Random();
	
	public TarczaCienia(RpgPlayer rpgPlayer, EpicRune rune) {
	...
	public void castSpell() {
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.TARCZA_CIENIA,
				player,
				target -> {
					target.getWorld().playSound(target.getLocation(), Sound.ENTITY_ILLUSIONER_PREPARE_MIRROR, 1.2f, 0.8f);
				}, 
				target -> {
					target.getWorld().playSound(target.getLocation(), Sound.ENTITY_GHAST_SHOOT, 1, 1.7f);
				...
				new TimingRuneEffect(4, target -> {
					...
					_loc.getWorld().spawnParticle(Particle.SQUID_INK, _loc, 6, 0.4f, 0.8f, 0.4f, force);
				}));
	}
