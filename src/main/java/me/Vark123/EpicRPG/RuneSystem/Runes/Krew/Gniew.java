...
public class Gniew extends ACastableRune {

	private Random rand = new Random();
	
	public Gniew(RpgPlayer rpgPlayer, EpicRune rune) {
	...
	public void castSpell() {
		BufferRuneTemplate.castEffect(
				this,
				rune.getName(),
				EpicModifierTypes.GNIEW,
				player,
				target -> {
					target.getWorld().playSound(target.getLocation(), Sound.ENTITY_ZOMBIFIED_PIGLIN_ANGRY, 1.2f, 0.8f);
				}, 
				target -> {
				...
				new TimingRuneEffect(4, target -> {
					...
					for(int i = 0; i < 4; ++i) {
						...
						_loc.getWorld().spawnParticle(Particle.RAID_OMEN, _loc.clone().add(x,y,z), 0,
								0, 1, 0, force);
					}
					for(int i = 0; i < 3; ++i) {
						double radius = rand.nextDouble(0.8);
						double angle = rand.nextDouble(Math.PI*2);
						double force = rand.nextDouble(0.03, 0.16);
						
						double x = radius * Math.sin(angle);
						double y = rand.nextDouble(0.2);
						double z = radius * Math.cos(angle);
						
						_loc.getWorld().spawnParticle(Particle.TRIAL_SPAWNER_DETECTION, _loc.clone().add(x,y,z), 0,
								0, 1, 0, force);
					}
				}));
	}
