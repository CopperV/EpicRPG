...
public class UderzenieBurzy extends ARune {

	private List<Entity> shooted;
	
	public UderzenieBurzy(ItemStackRune dr, Player p) {
		super(dr, p);
	...
	public void castSpell() {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				p.getWorld().spawnParticle(Particle.CLOUD, loc, 10, 1F,1F,1F,0.3F);
//				loc.getWorld().playSound(loc, Sound.FIRE_IGNITE, 1, 1);
//				for(Entity e:loc.getChunk().getEntities()) {
//					if(e.getLocation().distance(loc) < 5.0 || e.getLocation().add(0, 1, 0).distance(loc) < 5.0 || e.getLocation().add(0, 1, 0).distance(loc) < 5.0) {
...
//				loc.getWorld().playSound(loc, Sound.FIRE_IGNITE, 1, 1);
	...
	private boolean odrzutaZakoncz(Location loc, Vector vec) {
		...
		for(Entity e:loc.getWorld().getNearbyEntities(loc, 10, 10, 10)) {
			...
			AxisAlignedBB aabb2 = new AxisAlignedBB(loc.getX()-5, loc.getY()-5, loc.getZ()-5, loc.getX()+5, loc.getY()+5, loc.getZ()+5);
			if(aabb.c(aabb2)) {
				if(!e.equals(p) && e instanceof LivingEntity && !shooted.contains(e)) {
					shooted.add(e);
