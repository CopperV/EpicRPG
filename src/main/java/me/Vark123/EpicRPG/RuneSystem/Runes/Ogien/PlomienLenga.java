...
public class PlomienLenga extends ACastableRune {
	
	private IRuneHitCondition hitCondition;
	private BoundingBox boundingBox;
	private IRunePostDamageEffect hitEffect;

	public PlomienLenga(RpgPlayer rpgPlayer, EpicRune rune) {
		...
		boundingBox = new BoundingBox(
				-0.92, -0.92, -0.92, 
				0.92, 0.92, 0.92);
		hitEffect = new DamageBurnEffect(12, rune.getDamage() * 0.12, this);
	}
	...
	public void castSpell() {
		...
		ProjectileRuneTemplate.castProjectile(
				...
				boundingBox,
				loc -> {
					loc.getWorld().playSound(loc, Sound.ENTITY_GHAST_SHOOT, 1, 0.4f);
				}, 
				loc -> {
					loc.getWorld().spawnParticle(Particle.TRIAL_SPAWNER_DETECTION, loc, 4, 
							0.3f, 0.3f, 0.3f, 0.06f);
					loc.getWorld().spawnParticle(Particle.LAVA, loc, 7, 
							...
							_loc -> {
								...
								for(int i = 0; i < points; ++i) {
									...
									_loc.getWorld().spawnParticle(Particle.TRIAL_SPAWNER_DETECTION, _loc, 0,
											x, 0, z, 0.3f);
									_loc.getWorld().spawnParticle(Particle.LARGE_SMOKE, _loc, 0,
											x, 0, z, 0.13f);
								}
								_loc.getWorld().spawnParticle(Particle.LAVA, _loc, 40, 
										0.45f, 0.45f, 0.45f, 0.1f);
								_loc.getWorld().playSound(_loc, Sound.ENTITY_BLAZE_HURT, 1, 0.6f);
							}, 
							hitCondition, 
							(_loc, _e) -> {
