...
public class AttackNearestCommand extends ASummonCommand {
	...
	public void apply(Player owner, ActiveMob summon) {
		...
		loc.getWorld().playSound(loc, Sound.BLOCK_TRIAL_SPAWNER_SPAWN_ITEM, 1.2f, 1f);
		...
		for(int i = 0; i < points; ++i) {
			...
			p1.getWorld().spawnParticle(Particle.TRIAL_SPAWNER_DETECTION_OMINOUS, p1, 1, 0, 0, 0, 0);
		}
		for(int i = 0; i < 25; ++i) {
			...
			p1.getWorld().spawnParticle(Particle.TRIAL_SPAWNER_DETECTION_OMINOUS, p1, 0, 0, 1, 0, rand.nextFloat(0.03f, 0.15f));
		}
